...
exp_root_dir = os.path.join(root_path, 'experiments', 'Xerostomia_BCE_clinical_features')   # Change this one so that each experiment is put in its own folder!
...
scheduler_name = 'cosine' # 'cyclic'  # [None, 'cosine', 'cyclic', 'exponential', 'step', 'lr_finder', 'reduce_lr', 'batch_size',
...
base_lr = 1e-4  # (CylicLR, only if perform_lr_finder = False, see get_scheduler()) Minimum and starting learning rate.
