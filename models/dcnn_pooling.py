...
class conv_block(torch.nn.Module):
    def __init__(self, in_channels, filters, kernel_size, strides, pad_value, lrelu_alpha, use_activation,
        ...
        elif pooling == 'avg' or 'average':
            self.pool = torch.nn.AvgPool3d(kernel_size=(2, 2, 2))
        elif pooling == 'none' or pooling == None:
            self.pool = None
        else:
...
class DCNN_Pooling(torch.nn.Module):
    ...
    def __init__(self, n_input_channels, depth, height, width, n_features, num_classes, filters, kernel_sizes, strides,
                 pad_value, n_down_blocks, lrelu_alpha, dropout_p, pooling_conv_filters, perform_pooling, pooling,
                 linear_units,
        ...
        print(end_depth, end_height, end_width)
        ...
        for i in range(len(in_channels)):
            ...
            self.conv_blocks.add_module('conv_block%s' % i,
                                        conv_block(in_channels=in_channels[i], filters=filters[i],
                                                   ...
                                                   use_activation=use_activation, pooling=pooling, use_bias=use_bias))
        ...
        linear_units = [864] + linear_units   # BUG: have to find a different way to calulate the size of the flattening layer, if stride=1 and pooling is used
